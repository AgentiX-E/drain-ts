...
interface DatasetDescriptor {
  ...
  /** Enable param_count as second dimension in tree binning (AEL-inspired). */
  enableParamBinning?: boolean;
...
const DATASETS: DatasetDescriptor[] = [
  ...
  },
  {
    ...
    // Layer 1: Normalize duration format (<1 sec → <NUM>:<NUM>)
    // Layer 2: Remove KB parentheticals
    // Layer 3: AdjacentConstantFusion fuses constant adjacent tokens
    drainExtraDelimiters: [","],
    regexCollapsePatterns: [
      // Normalize "<1 sec" → "<NUM>:<NUM>" to match "00:01" format
  ...
  const miner = new TemplateMiner({
    config: TemplateMinerConfig.from({
      ...
      ...(ds.regexCollapsePatterns !== undefined
        ...
        : {}),
      ...(ds.enableParamBinning !== undefined
        ? { enableParamBinning: ds.enableParamBinning }
        : {}),
